...
hylidae.shp <- list.files("data/shp_hylidae",pattern = ".shp", recursive = TRUE, full.names = TRUE)

ranges_hylidae <- data.frame(especie = character(0),
                             lat_max = numeric(0),
                             lat_min = numeric(0),
                             area_occ = numeric(0),
...
for (shapefile in hylidae.shp) {
    ...
    if (nrow(dados_shapefile) > 0) {
        ...
        ranges_hylidae <- rbind(ranges_hylidae, novo_registro)
    }
}
...
write.csv(ranges_hylidae, "data/processed/ranges_hylidae.csv", row.names = F)
